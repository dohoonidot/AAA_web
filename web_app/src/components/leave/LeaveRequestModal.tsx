/**
 * 휴가 신청 모달
 * LeaveRequestDraftPanel과 100% 동일한 구현
 */
...
export default function LeaveRequestModal({
  ...
  // 폼 데이터 상태 (Zustand store 대신 로컬 state)
  ...
  // 로컬 상태 (LeaveRequestDraftPanel과 동일)
  ...
  // 초기 데이터 로드 (LeaveRequestDraftPanel과 동일)
  ...
  // 결재라인 저장 핸들러 (LeaveRequestDraftPanel과 동일)
  const handleSaveApprovalLine = async () => {
    ...
    try {
      ...
      const apiCcList = (ccList || []).map((item) => ({
        name: item.name,
        userId: item.userId || '',
  ...
  // 내년 정기휴가 체크박스 핸들러 (LeaveRequestDraftPanel과 동일)
  ...
  return (
    <>
                    ...
                    <Box>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 1 }}>
                        ...
                        <Typography sx={{ fontSize: '11px', fontWeight: 600, color: colorScheme.textColor }}>
                          선택된 승인자 ({approvalLine.length}명)
                          {isSequentialApproval && ' (순차결재)'}
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                        {approvalLine.map((approver, idx) => (
                          <Chip
                            key={idx}
      ...
      <ApproverSelectionModal
        ...
        initialSelectedApproverIds={approvalLine?.map((a) => a.approverId) || []}
